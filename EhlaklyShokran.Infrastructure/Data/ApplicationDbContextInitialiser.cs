        ...
        RoleManager<IdentityRole> roleManager)
    {
        ...
        public async Task TrySeedAsync()
        {
            ...
            if (!_context.Customers.Any())
            {
                ...
                [
                    Customer.Create(id: Guid.Parse("f522bbe5-e3b1-4e2c-a8a3-c41550dcf39d"), name: "John Doe", phoneNumber: "123456789", email: "john.doe@localhost").Value,
                Customer.Create(id: Guid.Parse("73a04dd3-c81a-4a54-9882-ef1017eb192d"), name: "Sarah Peter", phoneNumber: "987654321", email: "sarah.peter@localhost").Value,
            ]);
            }

            if (!_context.BarberTasks.Any())
            {
                _context.BarberTasks.AddRange([
                    BarberTask.Create(id: Guid.Parse("616aebb1-d515-4b40-8d47-8d5c0b67a313"), name: "Engine Oil Change", laborCost: 50.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min60, Cosmetics: [Cosmetic.Create(Guid.Parse("ec65225c-9066-4a1c-974f-f183c39fdd16"), "Engine Oil", 25.00m, 1).Value, Cosmetic.Create(Guid.Parse("62ad80e3-2cff-41af-ab40-16fab8db8b38"), "Oil Filter", 10.00m, 1).Value]).Value,
                BarberTask.Create(id: Guid.Parse("4fa0be55-06f6-4616-b086-e1f0c9354cd8"), name: "Brake Replacement", laborCost: 150.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min90, Cosmetics: [Cosmetic.Create(Guid.Parse("86375a12-715e-4aa4-aad9-c0f9ccf44a14"), "Brake Pads", 40.00m, 2).Value, Cosmetic.Create(Guid.Parse("526d89c3-a971-4ea7-ba15-de6b50b13c21"), "Brake Fluid", 15.00m, 1).Value]).Value,
                BarberTask.Create(id: Guid.Parse("a376b5d1-6b2d-4dd8-883e-d3d1721c1316"), name: "Tire Rotation", laborCost: 30.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min45, Cosmetics: [Cosmetic.Create(Guid.Parse("a46f974e-a198-4098-8a1f-6be6e68ec743"), "Tire Valve", 5.00m, 4).Value]).Value,
                BarberTask.Create(id: Guid.Parse("a770cc6e-0c8b-4ac5-9ee6-6928682bd47e"), name: "Battery Replacement", laborCost: 70.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min30, Cosmetics: [Cosmetic.Create(Guid.Parse("d4fd3255-29dc-4d45-9d87-f58ab98bc28b"), "Car Battery", 120.00m, 1).Value]).Value,
                BarberTask.Create(id: Guid.Parse("e4c2b675-4a60-488f-a7b4-61966e7e80e3"), name: "Wheel Alignment", laborCost: 80.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min60, Cosmetics: [Cosmetic.Create(Guid.Parse("fa3b9a7e-1c2d-4e3f-9b8a-0c1d2e3f4a5b"), "Alignment Shim Kit (per wheel)", 5.00m, 4).Value]).Value,
                BarberTask.Create(id: Guid.Parse("1cb1608c-3bc7-4325-99c3-8244c0fb412f"), name: "Air Conditioning Recharge", laborCost: 100.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min30, Cosmetics: [Cosmetic.Create(Guid.Parse("526dca0a-d236-47d3-8e8f-c83d555b2de9"), "Refrigerant", 50.00m, 1).Value]).Value,
                BarberTask.Create(id: Guid.Parse("a8e9b4e0-8581-40df-967d-51a0f4fabc0e"), name: "Spark Plug Replacement", laborCost: 90.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min60, Cosmetics: [Cosmetic.Create(Guid.Parse("019f5eab-a8a5-44d4-92b3-1f998e3f10c2"), "Spark Plug", 10.00m, 4).Value]).Value,
                BarberTask.Create(id: Guid.Parse("90f2f3ef-3357-439e-9689-628aa08200c1"), name: "Engine Diagnostic", laborCost: 120.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min120, Cosmetics: [Cosmetic.Create(Guid.Parse("c3d4e5f6-a7b8-9c0d-1e2f-3a4b5c6d7e8f"), "Smoke Leak Detector Fluid Cartridge", 20.00m, 1).Value]).Value,
                BarberTask.Create(id: Guid.Parse("d124651e-ca72-467e-ba28-81ea4a2080bc"), name: "Timing Belt Replacement", laborCost: 200.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min120, Cosmetics: [Cosmetic.Create(Guid.Parse("06b764a0-73a2-4c37-b279-adae3856499c"), "Timing Belt", 75.00m, 1).Value]).Value,
                BarberTask.Create(id: Guid.Parse("cee9b309-8620-4028-8d38-2532771ab3ea"), name: "Transmission Fluid Change", laborCost: 100.00m, estimatedDurationInMins: ServiceDurationInMinutes.Min45, Cosmetics: [Cosmetic.Create(Guid.Parse("0a8b0c19-873a-4da0-811b-45ff85bca0ed"), "Transmission Fluid", 60.00m, 1).Value]).Value
                ]);
            }
            ...
            {
                var BarberTasks = _context.BarberTasks.ToList();
                ...
                while (startDate < endDate)
                {
                    foreach (Spot spot in spots)
                    {
                        ...
                        {
                            var selectedTask = BarberTasks
                                                .DistinctBy(t => t.Id)
                                                .OrderBy(_ => Guid.NewGuid())
                                                .Take(Random.Shared.Next(1, Math.Min(4, BarberTasks.Select(t => t.Id).Distinct().Count())))
                                                .ToList();
                            ...
                            var availableVehicle = customers
                ...
                var BarberTasksForFirstOrder = _context.BarberTasks
                 .OrderBy(_ => Guid.NewGuid())
                 .Take(2)
                 .ToList();
                ...
                var workOrderStartingNow = WorkOrder.Create(
                    Guid.NewGuid(),
                    _context.Customers.OrderBy(_ => Guid.NewGuid()).First().Id,
                    startTimeFirstOrder,
                    startTimeFirstOrder.AddMinutes(BarberTasksForFirstOrder.Sum(rt => (int)rt.EstimatedDurationInMins)),
                    Guid.Parse(labor01.Id),
                    Spot.A,
                    BarberTasksForFirstOrder).Value;

                workOrderStartingNow.UpdateState(WorkOrderState.InProgress);

                var BarberTasksEndingNow = _context.BarberTasks
          .First(rt => rt.EstimatedDurationInMins == ServiceDurationInMinutes.Min60);
                ...
                var endTimeSecondOrder = roundedStart.AddMinutes((int)BarberTasksEndingNow.EstimatedDurationInMins);
